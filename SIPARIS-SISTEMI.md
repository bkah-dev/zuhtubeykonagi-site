...
İlk migration çalışmadan önce Supabase panelinde `Authentication > Users` bölümünden en az bir personel hesabı oluşturulmalıdır. İlk kurulum mevcut kullanıcıları `staff_members` tablosuna güvenli biçimde ekler. Daha sonra açılan hesaplar ayrıca personel listesine alınmalıdır. Personel ekranında kullanıcı kaydı özelliği yoktur; yalnızca hem Supabase kullanıcısı olan hem de personel listesine alınan hesaplar verilere erişebilir.
